...
// status가 rejected/accepted인 지원자 중에서 jobSeekerId로 jobId를 찾고, 그에 해당하는 Job 테이블의 정보들을 불러오기
module.exports = async (req, res) => {

  const { jobSeekerId } = req.query

  let applyStatus = await Applicant.findAll(
    {where: {jobSeekerId, [Op.or]: [{status: 'rejected'}, {status: 'accepted'}]}, attributes: ['status']},  
  )

  // Applicant 테이블에서 query params의 jobSeekerId에 해당하는 JobId를 모두 찾기
  // 쿼리문 예시 : SELECT jobId AS id FROM Applicant WHERE jobSeekerId=1
  Applicant.findAll(
    {where: {jobSeekerId, [Op.or]: [{status: 'rejected'}, {status: 'accepted'}]}, attributes: [['jobId', 'id']]},  
  )
  .then((data)=>{
