...
module.exports = async (req, res) => {
  ...
  if(!jobId && !jobSeekerId){
    ...
    if (!applicantInfo) {
      ...
      Applicant.update({status: 'accepted'}, {where: {jobId, jobSeekerId}})
      .then((date) => {
        ...
        let mailOptions = {
          ...
          subject: `[HomeTownAlba] ${jobInfo.dataValues.companyName}의 지원 승인`,
          html: emailTemplete,
        };
        ...
        res.status(201).send("지원을 승인했습니다")
      })
      .catch((err)=> {
        console.log(err);
