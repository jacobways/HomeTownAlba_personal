...
export default function Map() {
  ...
  //로그인 정보 불러오는 이팩트
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_SERVER_URL}/jobSeeker`, { withCredentials: true })
    .then(res => {
      if (res.data.user) {
  ...
  // 지도를 표시하는 useEffect
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_SERVER_URL}/job`, { withCredentials: true })
    .then(res => {
      if (mapLevel<=4) {
    ...
    .then(data => {
      ...
      if (!(applyLocation.length === 0)) {  // 지원 or 지원 취소한 일자리 위치가 state에 있는 경우
      ...
      } else if (searchLocation) {  // 장소 검색을 한 경우
      ...
      } else if (!(mapLocation.length===0)) {  // 현재 위치가 state에 있는 경우
          ...
          if (status === kakao.maps.services.Status.OK) { // 정상적으로 검색이 완료됐으면
            ...
            .then(applicant => {
              ...
              function MakeMarkerEvent (marker, buttonContent, applyHandler) { 
                ...
                contentClick.style.cssText = "width: 230px; text-align:center; padding:6px 0; 1px 0;";
              ...
              if (applicant.data.data) {  // 이미 지원 한 일자리의 경우 마커 생성
