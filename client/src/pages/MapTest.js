...
export default function Map() {
  const [isOpen, setIsOpen] = useState(false); // 목록 지원자 모달
  ...
  const [placeFliterData, setPlaceFliterData] = useState([]); // 장소 검색 결과 데이터
  //---------------------바뀐부분
        ...
        if (applyLocation.length === 0) {
          ...
          if (place) {
            axios
              ...
              .then(data => {
                setPlaceFliterData(data);
                //---------------------바뀐부분
                setListState(Array(data.length).fill(false));
                //--------------------
                        ...
                        axios
                          ...
                          .then(applicant => {
                            if (applicant.data.data) {
                              ...
                              day.textContent = `근무요일 : ${JSON.parse(
                                data[i].day
                              )}`;
                ...
                .then(applicant => {
                  if (applicant.data.data) {
                    ...
                    let infowindowHover = new kakao.maps.InfoWindow({
                      content: `<div style="width:230px;text-align:center;padding:6px 0;"><div>근무요일 : ${JSON.parse(
                        data[i].day
                      )}</div><div>하루 근무시간 : ${data[i].startTime}~${
                        data[i].endTime
                      }
                  </div><div>월급여 : ${
                    data[i].monthlyWage
