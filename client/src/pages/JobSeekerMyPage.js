...
export default function JobSeekerMyPage() {
  ...
  const updateCareer = (idx, id) => {
    ...
    axios
      .patch("http://localhost:5000/career", {
        id,
        jobSeekerId,
        company,
        period,
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:5000/career/2`, { withCredentials: true })
      .then((res) => {
      ...
      {!jobSeekerInfoUpdating ? (
        <>
          ...
          >
            ...
            <span>성별 : {gender}</span>
            <span>프로필 사진</span>
            <img width="200px" src={image} alt="kk" />
            <span>질문 : {question}</span>
            <button onClick={jobSeekerHandler}>수정</button>
          </div>
        </>
      ) : (
        <>
          <input name="name" type="text" onChange={nameHandler} value={name} />
